...
This white paper emphisizes the potential of quantisation and shows how to limit the noise from quantisation. We consider here the quantisation of neural networks themselves, not the data.
...
Quantisation is a technique of model compression where we change existing coding to a lower precision. For instance, we may convert FP32(32-bits floating numbers) into INT8(integers on 8 bits) BUT we may keep the final accuracy of networks used.

Chosing a quantization is chosing a scheme, a granularity and a bit-width. Quantization can be partial, asymetric and change over the neural networks layers. Howerver, this article only developpes the  homogeneous bit-width case.
...
Each layer can be quantized differently, and it depends on their use. Most used layers are quoted in this paper, to choose a quantization scheme.

useful link to get the basis: 
